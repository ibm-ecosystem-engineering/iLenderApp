...
export class CustomerService extends CommonAppService{
  ...
  getAll(): Observable<any> {
    return this.http.get(this.getURL() + '/api/core/customers/', this.getHttpHeaders()).pipe(map(this.extractData));
    // let url =  '/api2/customer';
    // return this.http.get(url, this.getHttpHeaders()).pipe(map(this.extractData));
   }

  getById(id): Observable<any> {
    return this.http.get(this.getURL() + '/api/core/customerProfile/'+ id, this.getHttpHeaders()).pipe(map(this.extractData));
  }
  ...
  public  getURL() {
    return this.urlService.getLocalHostUrl() ;
  }
}
